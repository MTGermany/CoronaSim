...
echo "Deactivated for the time being! Use old data from data2 until further notice!"
exit
...
perl -i -p -e 's/\<td\>([0-9]+)\<\/td\>.*\n/<td>\1<\/td>/g' data/RKI_*_selected.csv
...
perl -i -p -e 's/(\<td\>[0-9]+\<\/td\>\s+){363}(\<td\>[0-9]+\<\/td\>\s+)(\<td\>[0-9]+\<\/td\>\s+)*/\2/g' data/RKI_*_Kreis14521.csv

perl -i -p -e 's/(\<td\>[0-9]+\<\/td\>\s+){367}(\<td\>[0-9]+\<\/td\>\s+)(\<td\>[0-9]+\<\/td\>\s+)*/\2/g' data/RKI_*_Kreis14612.csv

perl -i -p -e 's/(\<td\>[0-9]+\<\/td\>\s+){371}(\<td\>[0-9]+\<\/td\>\s+)(\<td\>[0-9]+\<\/td\>\s+)*/\2/g' data/RKI_*_Kreis14628.csv

# strip tr and td tags

perl -i -p -e 's/^.*tr id=.*\n//g' data/RKI_*_Kreis*.csv
perl -i -p -e 's/^.*td id=.*\n//g' data/RKI_*_Kreis*.csv

# bring cases files to beginning of array of list entries
# {"date": "2020-03-05", "cases": 1,

perl -i -p -e 's/^.*\<td\>(\d{4}-\d{2}-\d{2}).*\n/\{\"date\": \"\1\",/g' data/RKI_cases_Kreis*.csv
perl -i -p -e 's/\,\s+\<td\>(\d+).*$/, \"cases\": \1,/g' data/RKI_cases_Kreis*.csv
...
sed -i '1,/^.*thead/d' data/RKI_*_Kreis*.csv
...
perl -i -p -e "s/^.*\<tbody.*$/dataRKI_string='\{\"LK_Erzgebirgskreis\":  [/g" data/RKI_Kreis14521.json
perl -i -p -e "s/^.*\<tbody.*$/\"SK_Dresden\":  [/g" data/RKI_Kreis14612.json
perl -i -p -e "s/^.*\<tbody.*$/\"LK_Osterzgebirge\":  [/g" data/RKI_Kreis14628.json

perl -i -p -e "s/^.*\<\/tbody.*$/]\}/g" data/RKI_Kreis*.json
